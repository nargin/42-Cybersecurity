...
Options:\
	...
	-k, --key	generate a new key with the file generated by the -g option.\
...
fn	arguments_check(args: &Vec<String>) {
	...
	if args[1] == "-g" || args[1] == "--generate" {
		...
		println!("File created at {}", std::fs::canonicalize("test.tmp").expect("Unable to get file path").display());
		...
		if hex_key.len() < 64 {
			error!("Invalid key length: {}", hex_key.len());
			println!("Expected 64 hexadecimal characters but got {}", hex_key.len());
			std::process::exit(1);
		}
	}
